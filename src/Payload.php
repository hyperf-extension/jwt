...
class Payload implements ArrayAccess, Arrayable, Countable, Jsonable, JsonSerializable
{
     ...
     *
     * @var \HyperfExtension\Jwt\Claims\Collection
     */
    private $claims;

    /**
     * @var \HyperfExtension\Jwt\Contracts\PayloadValidatorInterface
     */
    private $validator;
    ...
    public function __call(string $method, array $parameters)
    {
        if (preg_match('/get(.+)\b/i', $method, $matches)) {
            foreach ($this->claims as $claim) {
                if (get_class($claim) === 'HyperfExtension\\Jwt\\Claims\\' . $matches[1]) {
